...
/** Body Stone — passively regenerates health using player EMC. */
public class BodyStoneItem extends ActiveEmcItem {
    public BodyStoneItem(Properties p) { super(p); }
    @Override public long getEmcPerTick() { return 8L; }
    ...
    public void onTick(Player player, ItemStack stack, PlayerDataService service) {
        ...
        if (player.getHealth() < player.getMaxHealth() && player.level().getGameTime() % 20 == 0) {
    ...
    @Override public InteractionResult use(Level l, Player p, net.minecraft.world.InteractionHand h) {
        ...
        if (!l.isClientSide()) p.sendSystemMessage(Component.translatable("item.projecte.body_stone." + (was ? "off" : "on")));
