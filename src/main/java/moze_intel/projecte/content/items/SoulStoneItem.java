...
/** Soul Stone — passively regenerates hunger/saturation using player EMC. */
public class SoulStoneItem extends ActiveEmcItem {
    public SoulStoneItem(Properties p) { super(p); }
    ...
    @Override public InteractionResult use(Level l, Player p, net.minecraft.world.InteractionHand h) {
        ...
        if (!l.isClientSide()) p.sendSystemMessage(Component.translatable("item.projecte.soul_stone." + (was ? "off" : "on")));
